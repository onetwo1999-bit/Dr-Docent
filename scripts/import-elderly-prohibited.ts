/**
 * DUR 노인금기 품목 → drug_master 업로드
 * - 원본: scripts/dur_elderly_prohibited_utf8.csv (593건, UTF-8)
 * - item_seq 기준으로 drug_master 행을 찾아 업데이트, 없으면 insert
 * - 노인금기 = 노인에게 처방 자체를 피해야 하는 약물 (노인주의보다 강한 등급)
 *
 * 실행 전: Supabase에서 supabase/drug_master-elderly-prohibited.sql 실행 필요
 * 실행: node --import tsx scripts/import-elderly-prohibited.ts
 */
...
const CSV_PATH = path.join(__dirname, 'dur_elderly_prohibited_utf8.csv')
const BATCH_SIZE = 50

type Row = {
  item_seq: string; item_name: string; main_ingr_name: string
  company_name: string; prohibited_detail: string
}
...
async function readCsv(): Promise<Row[]> {
  ...
  for await (const line of rl) {
    ...
    // company_name(5), notice_date(6), notice_no(7), prohibited_detail(8), reimbursement(9)
    rows.push({
      item_seq:          cols[0]?.trim(),
      item_name:         cols[1]?.trim(),
      main_ingr_name:    cols[2]?.trim(),
      company_name:      cols[5]?.trim(),
      prohibited_detail: cols[8]?.trim(),
    })
  }
    ...
    for (const row of batch) {
      ...
      if (id) {
        const { error } = await supabase.from('drug_master').update({
          elderly_prohibited:        true,
          elderly_prohibited_detail: row.prohibited_detail || null,
        }).eq('id', id)
