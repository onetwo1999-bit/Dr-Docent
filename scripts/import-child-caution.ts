/**
 * DUR 어린이주의약물 업로드 스크립트
 * 소스: scripts/dur_child_caution_utf8.csv
 * 대상: drug_master (item_seq 기준 upsert)
 *
 * 실행: npx tsx scripts/import-child-caution.ts
 */
...
const CSV_PATH = path.resolve(__dirname, 'dur_child_caution_utf8.csv')
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    const makePayload = (r: CsvRow) => ({
      ...
      child_caution:        true,
      child_caution_detail: r.child_caution_detail || null,
