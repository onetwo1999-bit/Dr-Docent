/**
 * DUR 임부금기약물 업로드 스크립트 (2024-06-25 버전)
 * 소스: scripts/dur_preg_contraindication2_utf8.csv
 * 대상: drug_master (item_seq 기준 upsert)
 *
 * 실행: npx tsx scripts/import-preg-contraindication2.ts
 */
...
const CSV_PATH = path.resolve(__dirname, 'dur_preg_contraindication2_utf8.csv')
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    const makePayload = (r: CsvRow) => ({
      ...
      preg_contraindication_grade: r.preg_contraindication_grade || null,
      preg_contraindication_detail:r.preg_contraindication_detail || null,
    })
