/**
 * DUR 효능군중복주의 품목 → drug_master 업로드
 * - 원본: scripts/dur_therapeutic_duplication_utf8.csv (12,147건, UTF-8)
 * - item_seq 기준으로 drug_master 행을 찾아 업데이트, 없으면 insert
 * - 원본 파일의 성분코드↔성분명 헤더 뒤바뀜은 변환 시 이미 교정됨
 *
 * 실행 전: Supabase에서 supabase/drug_master-therapeutic-duplication.sql 실행 필요
 * 실행: node --import tsx scripts/import-therapeutic-duplication.ts
 */
...
const CSV_PATH = path.join(__dirname, 'dur_therapeutic_duplication_utf8.csv')
const BATCH_SIZE = 50

type Row = {
  item_seq: string; item_name: string; main_ingr_name: string; company_name: string
  therapeutic_group: string; group_classification: string
}
...
async function readCsv(): Promise<Row[]> {
  ...
  for await (const line of rl) {
    ...
    // generic_code(5), company_name(6), therapeutic_group(7), group_classification(8),
    // notice_date(9), notice_no(10), reimbursement(11)
    rows.push({
      item_seq:             cols[0]?.trim(),
      item_name:            cols[1]?.trim(),
      main_ingr_name:       cols[2]?.trim(),
      company_name:         cols[6]?.trim(),
      therapeutic_group:    cols[7]?.trim(),
      group_classification: cols[8]?.trim(),
    })
  }
    ...
    for (const row of batch) {
      ...
      if (id) {
        const { error } = await supabase.from('drug_master').update({
          therapeutic_duplication_caution: true,
          therapeutic_group:               row.therapeutic_group || null,
          group_classification:            row.group_classification || null,
        }).eq('id', id)
