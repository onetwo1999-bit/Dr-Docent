/**
 * DUR 임부금기 품목 → drug_master 업로드
 * - 원본: scripts/dur_pregnancy_contraindication_utf8.csv (19,015건, UTF-8)
 * - item_seq 기준으로 drug_master 행을 찾아 preg_contraindication 관련 컬럼 업데이트
 * - 없는 item_seq는 신규 insert
 * - 금기등급: 1=절대금기, 2=안전성 미확립, M=병용금기
 *
 * 실행 전: Supabase에서 supabase/drug_master-pregnancy-contraindication.sql 실행 필요
 * 실행: node --import tsx scripts/import-pregnancy-contraindication.ts
 */
...
const CSV_PATH = path.join(__dirname, 'dur_pregnancy_contraindication_utf8.csv')
const BATCH_SIZE = 50
...
type CautionRow = {
  ...
  company_name: string
  preg_contraindication_grade: string
  preg_contraindication_detail: string
}
...
async function readCsv(): Promise<CautionRow[]> {
  const rows: CautionRow[] = []
  ...
  for await (const line of rl) {
    ...
    rows.push({
      ...
      preg_contraindication_grade: cols[8]?.trim() ?? '',
      preg_contraindication_detail:cols[9]?.trim() ?? '',
    })
  }
...
async function main() {
  ...
  rows.forEach(r => { gradeDist[r.preg_contraindication_grade] = (gradeDist[r.preg_contraindication_grade] ?? 0) + 1 })
  console.log('금기등급 분포:', gradeDist)
    ...
    for (const row of batch) {
      ...
      if (existingId) {
        const { error } = await supabase
          .from('drug_master')
          .update({
            preg_contraindication: true,
            preg_contraindication_grade: row.preg_contraindication_grade || null,
            preg_contraindication_detail: row.preg_contraindication_detail || null,
          })
          .eq('id', existingId)
