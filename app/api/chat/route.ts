...
`
  ...
  if (drugContext) {
    systemPrompt += `\n## [필수 — 식약처 의약품 공식 데이터]\n`
    systemPrompt += `아래는 식품의약품안전처(MFDS) 공공데이터(성분 상세 조회) 또는 우리 DB(drug_master)에서 가져온 **공식 의약품 정보**야. 제품명(prduct)·성분명(mtral_nm)이 각 항목에 포함돼.\n`
    systemPrompt += `\`\`\`\n${drugContext}\n\`\`\`\n\n`
    systemPrompt += `- **검색 결과 매핑**: 위 블록의 제품명·성분명을 그대로 사용해. 성분 분석·주성분 설명 시 MTRAL_NM(성분명) 값을 써.\n`
    systemPrompt += `- **건강 데이터 결합 분석**: 위 의약품 데이터와 **이 프롬프트 상단의 선생님 건강 프로필**(만 나이·성별·기저 질환·복용 약물)을 반드시 결합해서 답변해. 예: 38세 여성, 고혈압·갑상선 등 기저 질환이 있으면 해당 약과의 상호작용·주의사항을 맞춤형으로 짚어 주고, 연령·성별에 맞는 복용 가이드를 제시해.\n`
    ...
    systemPrompt += `- 행동 지침에는 유저가 **다음 단계로 할 구체적 행동**을 제시해. 예: "복용 전 약사 선생님께 질문할 때 '이 약과 혈압약을 함께 먹어도 될까요?'라고 여쭤보시면 좋아요", "갑상선 검사 결과지를 지참하고 병원 상담 시 보여 드리세요" 등.\n`
