...
`
  if (drugContext) {
    systemPrompt =
      `[Strict RAG Mode — 최우선]\n` +
      `DB 검색 결과가 1건이라도 제공되었으므로, **AI의 내부 지식(학습 데이터) 사용을 엄격히 금지**한다.\n` +
      `- 효능·용도·성분·주의사항·상호작용 등 **모든 의약품 관련 설명은 위에 제공된 DB 블록의 원문만** 사용할 것.\n` +
      `- "진통제", "해열제", "소화제" 등 **제품 분류나 용도는 DB의 main_ingredient·ee_doc_data에 적힌 내용으로만** 판단하고, 학습 데이터로 추론하지 마.\n` +
      `- 데이터에 없는 효능·용도는 절대 언급하지 말고, "현재 조회된 데이터에는 해당 정보가 없습니다"로만 안내할 것.\n\n` +
      systemPrompt
  }
...
`
  ...
  if (drugContext) {
    ...
    systemPrompt += `- **성분 매칭 강조 (필수)**: 답변 시 **반드시 DB의 main_ingredient(주성분)를 먼저 언급**한 뒤, 그 성분을 기반으로 효능을 설명해. 예: "이 제품은 DB에 등재된 주성분이 [건조수산화알루미늄겔, 탄산수소나트륨 등]이에요. 따라서 위 블록에 적힌 효능은 …"처럼, **성분 → 효능** 순서로만 서술하고, main_ingredient에 없는 용도(예: 진통·해열)는 절대 붙이지 마.\n`
    systemPrompt += `- **검색 결과 매핑**: 위 블록의 제품명·성분명·효능·주의사항을 그대로 사용해.\n`
    systemPrompt += `- **데이터 기반 경고만 허용**: 선생님 건강 프로필(예: 38세, 고혈압)과 결합할 때, **위 DB 블록(nb_doc_data·ee_doc_data·성분)에 실제로 적혀 있는 내용만** 근거로 행동 지침을 생성해. 예: nb_doc_data에 "나트륨 함량 주의" 또는 고혈압 관련 문구가 있을 때만 "나트륨 함량 주의"를 언급하고, DB에 없는 경고는 만들지 마.\n`
    systemPrompt += `- **건강 데이터 결합**: 위 의약품 데이터와 **선생님 건강 프로필**(만 나이·성별·기저 질환·복용 약물)을 결합할 때, **DB 원문에 나온 주의·금기·상호작용**만 인용해 맞춤형 안내를 해. 데이터에 없는 상호작용은 추론하지 말고, "해당 내용은 조회된 데이터에 없어 약사 선생님께 여쭤보시면 좋아요"로 마무리해.\n`
    ...
    systemPrompt += `- 답변에 반드시 **「행동 지침」** 섹션을 포함해. **DB 원문(ee_doc_data·nb_doc_data·main_ingredient)에 적힌 내용만** 인용한 데이터 기반 안내로 작성해.\n`
    systemPrompt += `- **효능·주의사항 활용**: 위 블록의 **효능**, **주의사항**이 있으면 그 문구를 인용해 구체적인 복약 안내를 해. 행동 지침은 **DB에 실제로 적힌 금기·상호작용·주의**만 언급하고, 유저가 다음에 할 구체적 행동(약사 질문, 검사 결과지 지참 등)을 제시해. DB에 없는 주의사항은 만들지 마.\n`
    systemPrompt += `- **[데이터 기반 안심 행동 지침]**: 학술 논문 블록이 있으면 논문(PMID)과 **DB 성분·주의사항**을 대조해 작성해. **고혈압·나트륨 함량 등 경고는 nb_doc_data·ee_doc_data에 해당 문구가 있을 때만** 넣고, 없으면 추론으로 추가하지 마.\n`
