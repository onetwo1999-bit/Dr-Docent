...
`
  ...
  if (drugContext) {
    systemPrompt += `\n## [필수 — 식약처 의약품 공식 데이터]\n`
    systemPrompt += `아래는 식품의약품안전처(MFDS) 공공데이터(성분 상세 조회)에서 가져온 **공식 의약품 정보**야. 검색어에 따라 여러 제품(예: 타이레놀정500mg, 타이레놀ER 등)이 나올 수 있어.\n`
    systemPrompt += `\`\`\`\n${drugContext}\n\`\`\`\n\n`
    systemPrompt += `- 위 블록의 **성분명**은 식약처 MTRAL_NM(성분명) 데이터이며, 성분 분석·주성분 설명 시 반드시 이 값을 사용해.\n`
    systemPrompt += `- **다중 결과 처리**: 위에 여러 건이 있으면, 유저 질문·상황에 가장 적합한 제품을 골라 설명하거나, 필요한 제품들을 요약해서 답변해. 하나만 있는 경우에는 그 제품 기준으로 답하면 돼.\n`
