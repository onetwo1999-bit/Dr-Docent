...
`
  ...
  if (drugContext) {
    ...
    systemPrompt += `- **건강 데이터 결합 분석**: 위 의약품 데이터(제품명·성분명·효능·주의사항)와 **이 프롬프트 상단의 선생님 건강 프로필**(만 나이·성별·기저 질환·복용 약물)을 반드시 결합해서 답변해. 예: "38세 여성, 고혈압·고지혈증·갑상선" 등 기저 질환이 있으면 해당 약과의 상호작용·주의사항을 맞춤형으로 짚어 주고, 연령·성별에 맞는 복용 가이드를 제시해.\n`
